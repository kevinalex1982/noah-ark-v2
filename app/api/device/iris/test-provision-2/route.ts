...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const memberPath = join(dataDir, 'iris_test_members', 'iris_data.json');
    ...
    const members = Array.isArray(raw.body) ? raw.body : [];
    if (members.length === 0) return NextResponse.json({ success: false, error: 'body数组为空', logs }, { status: 404 });
    ...
    const requestData: any = {
      ...
      singleIrisAllowed: 0, leftIrisImage: leftBmp, rightIrisImage: rightBmp, faceImage: member.faceImage || '',
    };
