  ...
  client.publish(topic, payload, { qos: 1 }, (err) => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        // 检查mqtt_logs
        const mqttLogs = fs.readdirSync(path.join(__dirname, '../data/mqtt_logs'));
        const latestLog = mqttLogs.filter(f => f.startsWith('passport-add')).sort().pop();
        if (latestLog) {
          console.log('');
          console.log('=== 最新MQTT日志 ===');
